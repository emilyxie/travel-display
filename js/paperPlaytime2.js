...
for (var i = 0; i < numBalls; i++) {
	...
	images.push(image.rasterize());
	image.remove();
	
	console.log(image.resolution);
	
	balls.push(new Ball(width, position, vector, i, images[i], landmarkData[i]));
}
...
var viewElement = document.getElementsByClassName("canvas")[0];
viewElement.addEventListener("mousemove", function(event) {
	var newX = event.x - viewElement.getBoundingClientRect().left;
	var newY = event.y - viewElement.getBoundingClientRect().top;
